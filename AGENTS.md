...
- **JSON context parsing (native):** `runjucks-napi` enables `fast-json` by default, so `renderStringFromJson` / `renderStringFromJsonBuffer` parse with `simd-json`. Prebuilds in `.github/workflows/npm-publish.yml` use the same `napi build` defaults as `npm run build`, so all published targets ship with that path. To build or debug with serde_json-only parsing, compile the crate with no default features, for example: `cargo check --manifest-path native/Cargo.toml -p runjucks-napi --no-default-features` (or add `--no-default-features` when invoking `cargo`/`napi` for `runjucks-napi`). CI runs that check so the opt-out configuration does not bitrot.
