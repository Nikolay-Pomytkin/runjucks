...
- [x] **Grow `tag_parity_cases.json`** — **P1** (shipped): 27 new vectors added covering `trimBlocks`/`lstripBlocks`, custom delimiters, for/else, tuple unpack, loop.*, set frame scoping, nested if, include ignore missing, extends+super, macro defaults/kwargs, import/from, filter blocks, inline if, whitespace control, and comments.
