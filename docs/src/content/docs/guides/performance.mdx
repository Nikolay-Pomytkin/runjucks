...
4. **JSON-string or Buffer ingress** — For large contexts, prefer **`renderStringFromJson`** (JSON text) or **`renderStringFromJsonBuffer`** (UTF-8 bytes) so Rust parses once instead of walking a live JS object over N-API. The default native build uses **`simd-json`** for that parse; maintainers can use `--no-default-features` on `runjucks-napi` for `serde_json` only. See [`RUNJUCKS_PERF.md`](https://github.com/Nikolay-Pomytkin/runjucks/blob/main/RUNJUCKS_PERF.md).
